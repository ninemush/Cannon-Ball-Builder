...
export function generateInitAllApplicationsXaml(targetFramework: TargetFramework = "Windows"): string {
  const isCrossPlatform = targetFramework === "Portable";
  const nsS = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsScg = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsSco = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
<!-- InitAllApplications.xaml — Auto-generated by CannonBall -->
<!-- REFramework: Initialize all applications needed for the process -->
<Activity mc:Ignorable="sap sap2010" x:Class="Initialize_Applications"
  xmlns="http://schemas.microsoft.com/netfx/2009/xaml/activities"
  xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"
  xmlns:mva="clr-namespace:Microsoft.VisualBasic.Activities;assembly=System.Activities"
  xmlns:s="clr-namespace:System;assembly=${nsS}"
    ...
    <TryCatch DisplayName="Safe Application Initialization">
      ...
      <TryCatch.Catches>
        <Catch x:TypeArguments="s:Exception">
          <ActivityAction x:TypeArguments="s:Exception">
            <ActivityAction.Argument>
              <DelegateInArgument x:TypeArguments="s:Exception" Name="initEx" />
            </ActivityAction.Argument>
            <Sequence DisplayName="Handle Init Error">
              <ui:LogMessage Level="Error" Message="[&quot;Application initialization failed: &quot;${concat}initEx.Message]" DisplayName="Log Init Error" />
              <Rethrow DisplayName="Re-throw Init Error" />
            </Sequence>
          </ActivityAction>
        </Catch>
      </TryCatch.Catches>
    </TryCatch>
    <ui:LogMessage Level="Info" Message="[&quot;All applications initialized successfully&quot;]" DisplayName="Log Init Complete" />
  </Sequence>
</Activity>`;
}

export function generateRetryCurrentTransactionXaml(targetFramework: TargetFramework = "Windows"): string {
  const isCrossPlatform = targetFramework === "Portable";
  const nsS = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsScg = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsSco = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
<!-- RetryCurrentTransaction.xaml — Auto-generated by CannonBall -->
<!-- REFramework: Manages retry logic for the current transaction on system exceptions -->
<Activity mc:Ignorable="sap sap2010" x:Class="RetryCurrentTransaction"
  xmlns="http://schemas.microsoft.com/netfx/2009/xaml/activities"
  xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"
  xmlns:mva="clr-namespace:Microsoft.VisualBasic.Activities;assembly=System.Activities"
  xmlns:s="clr-namespace:System;assembly=${nsS}"
  ...
  <Sequence DisplayName="Retry Current Transaction">
    ...
    <If DisplayName="Check if Queue Retry">
      <If.Condition>[in_QueueRetry]</If.Condition>
      <If.Then>
        <Sequence DisplayName="Queue Managed Retry">
          <ui:LogMessage Level="Info" Message="[&quot;Retry managed by Orchestrator Queue — skipping local retry counter&quot;]" DisplayName="Log Queue Retry" />
          <Assign DisplayName="Set Max Retry Not Reached">
            <Assign.To><OutArgument x:TypeArguments="x:Boolean">[out_MaxRetryReached]</OutArgument></Assign.To>
            <Assign.Value><InArgument x:TypeArguments="x:Boolean">[False]</InArgument></Assign.Value>
          </Assign>
        </Sequence>
      </If.Then>
      <If.Else>
        <Sequence DisplayName="Local Retry Logic">
          <Assign DisplayName="Increment Retry Counter">
            <Assign.To><OutArgument x:TypeArguments="x:Int32">[io_RetryNumber]</OutArgument></Assign.To>
            <Assign.Value><InArgument x:TypeArguments="x:Int32">[io_RetryNumber + 1]</InArgument></Assign.Value>
          </Assign>
          <If DisplayName="Check Max Retries">
            <If.Condition>[io_RetryNumber &gt;= int_MaxRetries]</If.Condition>
            <If.Then>
              <Sequence DisplayName="Max Retries Reached">
                <ui:LogMessage Level="Warn" Message="[&quot;Maximum retry count reached (&quot;${concat}io_RetryNumber.ToString()${concat}&quot;/&quot;${concat}int_MaxRetries.ToString()${concat}&quot;)&quot;]" DisplayName="Log Max Retries" />
                <Assign DisplayName="Set Max Retry Reached">
                  <Assign.To><OutArgument x:TypeArguments="x:Boolean">[out_MaxRetryReached]</OutArgument></Assign.To>
                  <Assign.Value><InArgument x:TypeArguments="x:Boolean">[True]</InArgument></Assign.Value>
                </Assign>
              </Sequence>
            ...
            <If.Else>
              <Sequence DisplayName="Retry Available">
                <ui:LogMessage Level="Info" Message="[&quot;Retrying transaction (&quot;${concat}io_RetryNumber.ToString()${concat}&quot;/&quot;${concat}int_MaxRetries.ToString()${concat}&quot;)&quot;]" DisplayName="Log Retry Attempt" />
                <Assign DisplayName="Decrement Transaction Number">
                  <Assign.To><OutArgument x:TypeArguments="x:Int32">[io_TransactionNumber]</OutArgument></Assign.To>
                  <Assign.Value><InArgument x:TypeArguments="x:Int32">[io_TransactionNumber - 1]</InArgument></Assign.Value>
                </Assign>
                <Assign DisplayName="Set Max Retry Not Reached">
...
export function generateRetryInitXaml(targetFramework: TargetFramework = "Windows"): string {
  const isCrossPlatform = targetFramework === "Portable";
  const nsS = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsScg = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsSco = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
<!-- RetryInit.xaml — Auto-generated by CannonBall -->
<!-- REFramework: Controls whether to retry initialization after a system exception -->
<Activity mc:Ignorable="sap sap2010" x:Class="RetryInit"
  xmlns="http://schemas.microsoft.com/netfx/2009/xaml/activities"
  xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"
  xmlns:mva="clr-namespace:Microsoft.VisualBasic.Activities;assembly=System.Activities"
  xmlns:s="clr-namespace:System;assembly=${nsS}"
  ...
  <Sequence DisplayName="Retry Init Logic">
    ...
    <Assign DisplayName="Increment Init Retry Counter">
      <Assign.To><OutArgument x:TypeArguments="x:Int32">[io_InitRetryCounter]</OutArgument></Assign.To>
      <Assign.Value><InArgument x:TypeArguments="x:Int32">[io_InitRetryCounter + 1]</InArgument></Assign.Value>
    </Assign>
    <If DisplayName="Check Init Retry Limit">
      <If.Condition>[io_InitRetryCounter &lt; int_MaxInitRetries]</If.Condition>
      <If.Then>
        <Sequence DisplayName="Allow Init Retry">
          <Assign DisplayName="Set Retry Init True">
            <Assign.To><OutArgument x:TypeArguments="x:Boolean">[io_RetryInit]</OutArgument></Assign.To>
            <Assign.Value><InArgument x:TypeArguments="x:Boolean">[True]</InArgument></Assign.Value>
          </Assign>
          <ui:LogMessage Level="Info" Message="[&quot;Init retry allowed — attempt &quot; &amp; io_InitRetryCounter.ToString() &amp; &quot; of &quot; &amp; int_MaxInitRetries.ToString()]" DisplayName="Log Init Retry" />
        </Sequence>
      </If.Then>
      <If.Else>
        <Sequence DisplayName="Init Retry Exhausted">
          <Assign DisplayName="Set Retry Init False">
            <Assign.To><OutArgument x:TypeArguments="x:Boolean">[io_RetryInit]</OutArgument></Assign.To>
            <Assign.Value><InArgument x:TypeArguments="x:Boolean">[False]</InArgument></Assign.Value>
          </Assign>
          <ui:LogMessage Level="Error" Message="[&quot;Init retries exhausted — max (&quot; &amp; int_MaxInitRetries.ToString() &amp; &quot;) reached&quot;]" DisplayName="Log Init Exhausted" />
        </Sequence>
      </If.Else>
    </If>
  </Sequence>
...
export function generateBuildTransactionDataXaml(targetFramework: TargetFramework = "Windows"): string {
  const isCrossPlatform = targetFramework === "Portable";
  const nsS = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsScg = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsSco = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
<!-- BuildTransactionData.xaml — Auto-generated by CannonBall -->
<!-- REFramework Dispatcher: Builds the transaction dataset to be dispatched to a queue -->
<Activity mc:Ignorable="sap sap2010" x:Class="BuildTransactionData"
  xmlns="http://schemas.microsoft.com/netfx/2009/xaml/activities"
  xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"
  xmlns:mva="clr-namespace:Microsoft.VisualBasic.Activities;assembly=System.Activities"
  xmlns:s="clr-namespace:System;assembly=${nsS}"
  ...
  <Sequence DisplayName="Build Transaction Data">
    ...
    <ui:LogMessage Level="Info" Message="[&quot;Transaction data built successfully&quot;]" DisplayName="Log Build Complete" />
  </Sequence>
</Activity>`;
}

export function generateCleanupAndPrepXaml(targetFramework: TargetFramework = "Windows"): string {
  const isCrossPlatform = targetFramework === "Portable";
  const nsS = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsScg = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsSco = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
<!-- CleanupAndPrep.xaml — Auto-generated by CannonBall -->
<!-- REFramework: Cleans up temporary files and prepares working directories -->
    ...
    <TryCatch DisplayName="Safe Cleanup">
      ...
      <TryCatch.Catches>
        <Catch x:TypeArguments="s:Exception">
          <ActivityAction x:TypeArguments="s:Exception">
            <ActivityAction.Argument>
              <DelegateInArgument x:TypeArguments="s:Exception" Name="cleanupEx" />
            </ActivityAction.Argument>
            <ui:LogMessage Level="Warn" Message="[&quot;Cleanup encountered an error: &quot;${concat}cleanupEx.Message]" DisplayName="Log Cleanup Error" />
          </ActivityAction>
        </Catch>
      </TryCatch.Catches>
    </TryCatch>
    <ui:LogMessage Level="Info" Message="[&quot;Cleanup and preparation completed&quot;]" DisplayName="Log Cleanup Complete" />
  </Sequence>
</Activity>`;
}

export function generateSendNotificationsXaml(targetFramework: TargetFramework = "Windows"): string {
  const isCrossPlatform = targetFramework === "Portable";
  const nsS = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsScg = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  const nsSco = isCrossPlatform ? "System.Runtime" : "System.Private.CoreLib";
  ...
  return `<?xml version="1.0" encoding="utf-8"?>
<!-- SendNotifications.xaml — Auto-generated by CannonBall -->
<!-- REFramework: Sends error or success notifications at the end of process execution -->
<Activity mc:Ignorable="sap sap2010" x:Class="SendErrorNotifications"
  xmlns="http://schemas.microsoft.com/netfx/2009/xaml/activities"
  xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006"
  xmlns:mva="clr-namespace:Microsoft.VisualBasic.Activities;assembly=System.Activities"
  xmlns:s="clr-namespace:System;assembly=${nsS}"
  ...
  xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml">
  <x:Members>
    <x:Property sap2010:Annotation.AnnotationText="Business Exception sent as a notification to the business." Name="in_BusinessException" Type="InArgument(ui:BusinessRuleException)" />
