...
import {
  ...
  CONTEXT_DERIVED_FALLBACK_REGISTRY,
  injectResolvedPropertyBindings,
...
<Activity mc:Ignorable="sap sap2010" x:Class="Main"
  ...
  <Sequence DisplayName="Main Sequence" sap2010:WorkflowViewState.IdRef="MainSequence_1">
    ...
    it("isGenericTypeDefault recognizes empty string as default for System.Object (Task #501 Phase 1)", () => {
      expect(isGenericTypeDefault("", "System.Object")).toBe(true);
      expect(isGenericTypeDefault("  ", "System.Object")).toBe(true);
      expect(isGenericTypeDefault("Nothing", "System.Object")).toBe(true);
      expect(isGenericTypeDefault("null", "System.Object")).toBe(true);
      expect(isGenericTypeDefault("SomeValue", "System.Object")).toBe(false);
    });

    it("injectResolvedPropertyBindings replaces empty-value attributes (Task #501 Phase 2)", () => {
      const content = `<ui:InvokeWorkflowFile DisplayName="Invoke Process" WorkflowFileName="" />`;
      const bindings = [{
        file: "Main.xaml",
        workflow: "Main",
        ...
        originalValue: "",
        resolvedValue: "AutoGenerated_Invoke_Invoke_Process.xaml",
        severity: "info" as const,
        packageModeOutcome: "bound" as const,
        occurrenceIndex: 0,
      }];
      const result = injectResolvedPropertyBindings(content, bindings, "Main.xaml");
      expect(result).toContain('WorkflowFileName="AutoGenerated_Invoke_Invoke_Process.xaml"');
      expect(result).not.toContain('WorkflowFileName=""');
    });

    it("injectResolvedPropertyBindings still inserts missing attributes when not present", () => {
      const content = `<ui:InvokeWorkflowFile DisplayName="Invoke Process" />`;
      const bindings = [{
        file: "Main.xaml",
        workflow: "Main",
        activityType: "InvokeWorkflowFile",
    ...
    it("injectResolvedPropertyBindings does not replace non-empty existing attributes", () => {
      const content = `<ui:InvokeWorkflowFile DisplayName="Invoke Process" WorkflowFileName="Existing.xaml" />`;
      const bindings = [{
        file: "Main.xaml",
        workflow: "Main",
        activityType: "InvokeWorkflowFile",
      ...
      const result = injectResolvedPropertyBindings(content, bindings, "Main.xaml");
      expect(result).toContain('WorkflowFileName="Existing.xaml"');
    });

    it("E2E: LogMessage.Message empty string triggers context-derived fallback via System.Object isGenericTypeDefault (Task #501 Phase 1)", () => {
      catalogServiceImport.load();
      const beforeXaml = `<ui:LogMessage DisplayName="Log Step Result" Level="Info" Message="" />`;
      expect(beforeXaml).toContain('Message=""');

      const entries = [{ name: "Main.xaml", content: beforeXaml }];
      const result = applyRequiredPropertyEnforcement(entries, true);

      const afterXaml = result.entries[0].content;
      expect(afterXaml).not.toContain('Message=""');

      const messageBinding = result.enforcementResult.requiredPropertyBindings.find(
        b => b.activityType === "LogMessage" && b.propertyName === "Message"
      );
      expect(messageBinding).toBeDefined();
      expect(messageBinding!.sourceBinding).toBe("context-derived");
      expect(messageBinding!.originalValue).toBe("");
      expect(messageBinding!.resolvedValue).toBeTruthy();
      expect(afterXaml).toContain(`Message="${messageBinding!.resolvedValue}"`);
    });

    it("E2E: InvokeWorkflowFile.WorkflowFileName empty attribute persists resolved fallback (Task #501 Phase 2)", () => {
      catalogServiceImport.load();
      const beforeXaml = `<ui:InvokeWorkflowFile DisplayName="Invoke Process" WorkflowFileName="" />`;
      expect(beforeXaml).toContain('WorkflowFileName=""');

      const entries = [{ name: "Main.xaml", content: beforeXaml }];
      const result = applyRequiredPropertyEnforcement(entries, true);

      const afterXaml = result.entries[0].content;
      expect(afterXaml).not.toContain('WorkflowFileName=""');

      const wfBinding = result.enforcementResult.requiredPropertyBindings.find(
        b => b.activityType === "InvokeWorkflowFile" && b.propertyName === "WorkflowFileName"
      );
      expect(wfBinding).toBeDefined();
      expect(wfBinding!.sourceBinding).toBe("context-derived");
      expect(wfBinding!.originalValue).toBe("");
      expect(wfBinding!.resolvedValue).toContain("AutoGenerated_Invoke_Invoke_Process.xaml");
      expect(afterXaml).toContain('WorkflowFileName="AutoGenerated_Invoke_Invoke_Process.xaml"');
    });

    it("E2E: LogMessage.Message before/after XAML evidence (Task #501 Phase 1 evidence)", () => {
      catalogServiceImport.load();
      const beforeXaml = `<ui:LogMessage DisplayName="Log Step Result" Level="Info" Message="" />`;
      const entries = [{ name: "Main.xaml", content: beforeXaml }];
      const result = applyRequiredPropertyEnforcement(entries, true);
      ...
      expect(beforeXaml).toMatch(/Message=""/);
      expect(afterXaml).not.toMatch(/Message=""/);
      expect(afterXaml).toMatch(/Message="[^"]+"/);
    });

    it("E2E: InvokeWorkflowFile.WorkflowFileName before/after XAML evidence (Task #501 Phase 2 evidence)", () => {
      catalogServiceImport.load();
      const beforeXaml = `<ui:InvokeWorkflowFile DisplayName="Invoke Process" WorkflowFileName="" />`;
      const entries = [{ name: "Main.xaml", content: beforeXaml }];
      const result = applyRequiredPropertyEnforcement(entries, true);
      ...
      expect(beforeXaml).toMatch(/WorkflowFileName=""/);
      expect(afterXaml).not.toMatch(/WorkflowFileName=""/);
      expect(afterXaml).toMatch(/WorkflowFileName="AutoGenerated_Invoke_Invoke_Process\.xaml"/);
    });
...
<Activity x:Class="Test"
  ...
  describe("fallback-eligible required-property resolution (Task #465)", () => {
    ...
    describe("decision framework integrity", () => {
      it("FallbackDecision type covers all five outcomes", () => {
        const decisions: FallbackDecision[] = ["source-bound", "expression-lowered", "fallback-applied", "context-derived-fallback", "blocked"];
    ...
    describe("context-derived fallback tier 7 (Task #496)", () => {
      it("CONTEXT_DERIVED_FALLBACK_REGISTRY has approved entries for LogMessage.Message and InvokeWorkflowFile.WorkflowFileName", () => {
        const logSpec = CONTEXT_DERIVED_FALLBACK_REGISTRY.find(
          s => s.activityType === "LogMessage" && s.propertyName === "Message"
        );
        expect(logSpec).toBeDefined();
        expect(logSpec!.propertyClass).toBe("LogMessage.Message");

        const invokeSpec = CONTEXT_DERIVED_FALLBACK_REGISTRY.find(
          s => s.activityType === "InvokeWorkflowFile" && s.propertyName === "WorkflowFileName"
        );
        expect(invokeSpec).toBeDefined();
        expect(invokeSpec!.propertyClass).toBe("InvokeWorkflowFile.WorkflowFileName");
      });

      it("LogMessage.Message context-derived fallback produces valid VB.NET expression with [Auto-generated] marker", () => {
        const spec = CONTEXT_DERIVED_FALLBACK_REGISTRY.find(
          s => s.activityType === "LogMessage" && s.propertyName === "Message"
        )!;
        const result = spec.deriveExpression({
      ...
      it("LogMessage.Message context-derived fallback falls back to workflowName when no displayName", () => {
        const spec = CONTEXT_DERIVED_FALLBACK_REGISTRY.find(
          s => s.activityType === "LogMessage" && s.propertyName === "Message"
        )!;
        const result = spec.deriveExpression({
        ...
        expect(result).toBeTruthy();
        expect(result).toContain("[Auto-generated]");
        expect(result).toContain("ProcessInvoice");
      });

      it("LogMessage.Message context-derived fallback VB-escapes quotes then XML-escapes", () => {
        const spec = CONTEXT_DERIVED_FALLBACK_REGISTRY.find(
          s => s.activityType === "LogMessage" && s.propertyName === "Message"
        )!;
        const result = spec.deriveExpression({
          displayName: 'Log "Special" & <Test>',
          fileName: "Main.xaml",
          workflowName: "Main",
          activityType: "LogMessage",
          propertyName: "Message",
      ...
      it("LogMessage.Message VB-escapes embedded quotes for valid VB.NET string literal", () => {
        const spec = CONTEXT_DERIVED_FALLBACK_REGISTRY.find(
          s => s.activityType === "LogMessage" && s.propertyName === "Message"
        )!;
        const result = spec.deriveExpression({
      ...
      it("InvokeWorkflowFile.WorkflowFileName derives from DisplayName with AutoGenerated_Invoke_ prefix", () => {
        const spec = CONTEXT_DERIVED_FALLBACK_REGISTRY.find(
          s => s.activityType === "InvokeWorkflowFile" && s.propertyName === "WorkflowFileName"
        )!;
        const result = spec.deriveExpression({
          displayName: "Process Invoice",
          fileName: "Main.xaml",
          workflowName: "Main",
          activityType: "InvokeWorkflowFile",
          propertyName: "WorkflowFileName",
        });
        expect(result).toBe("AutoGenerated_Invoke_Process_Invoice.xaml");
      });

      it("InvokeWorkflowFile.WorkflowFileName strips non-alphanumeric chars and falls back to workflowName", () => {
        const spec = CONTEXT_DERIVED_FALLBACK_REGISTRY.find(
          s => s.activityType === "InvokeWorkflowFile" && s.propertyName === "WorkflowFileName"
        )!;
        const result = spec.deriveExpression({
        ...
        expect(result).toBe("AutoGenerated_Invoke_ProcessData.xaml");
      });

      it("LogMessage.Message and InvokeWorkflowFile.WorkflowFileName policies have contextDerivedFallbackAllowed=true", () => {
        const logPolicy = getFallbackPolicy("LogMessage", "Message");
        expect(logPolicy).not.toBeNull();
        expect(logPolicy!.contextDerivedFallbackAllowed).toBe(true);
        expect(logPolicy!.fallbackAllowed).toBe(false);

        const invokePolicy = getFallbackPolicy("InvokeWorkflowFile", "WorkflowFileName");
        expect(invokePolicy).not.toBeNull();
        expect(invokePolicy!.contextDerivedFallbackAllowed).toBe(true);
        expect(invokePolicy!.fallbackAllowed).toBe(false);
      });

      it("non-approved properties do not have contextDerivedFallbackAllowed", () => {
        const approved = new Set(["LogMessage.Message", "InvokeWorkflowFile.WorkflowFileName"]);
        const nonApproved = FALLBACK_ELIGIBLE_POLICIES.filter(p => !approved.has(p.propertyClass));
        for (const policy of nonApproved) {
          expect(policy.contextDerivedFallbackAllowed ?? false).toBe(false);
        }
      });

      it("context-derived fallback reduces execution_blocking defects for LogMessage.Message in enforcer", () => {
        catalogServiceImport.load();
        if (!catalogServiceImport.isLoaded()) return;
        const xamlWithMissingMessage = makeValidXaml("Main",
          '<ui:LogMessage Level="Info" DisplayName="Log Process Start" />'
        );
        const entries = [{ name: "Main.xaml", content: xamlWithMissingMessage }];
        const result = enforceRequiredProperties(entries, true);
        const logMessageDefects = result.unresolvedRequiredPropertyDefects.filter(
          d => d.activityType === "LogMessage" && d.propertyName === "Message"
        );
        expect(logMessageDefects.length).toBe(0);

        const contextDerivedBindings = result.requiredPropertyBindings.filter(
          b => b.activityType === "LogMessage" && b.propertyName === "Message" && b.sourceBinding === "context-derived"
        );
        expect(contextDerivedBindings.length).toBe(1);
        expect(contextDerivedBindings[0].resolvedValue).toContain("[Auto-generated]");
        expect(contextDerivedBindings[0].resolvedValue).toContain("Log Process Start");
      });

      it("context-derived fallback diagnostics record decision correctly", () => {
        catalogServiceImport.load();
        if (!catalogServiceImport.isLoaded()) return;
        const xamlWithMissingMessage = makeValidXaml("Main",
          '<ui:LogMessage Level="Info" DisplayName="Test Activity" />'
        );
        const entries = [{ name: "Main.xaml", content: xamlWithMissingMessage }];
        const result = enforceRequiredProperties(entries, true);
        const contextDiag = result.fallbackResolutionDiagnostics.filter(
          d => d.activityType === "LogMessage" && d.decision === "context-derived-fallback"
        );
        expect(contextDiag.length).toBe(1);
        expect(contextDiag[0].fallbackApplied).toBe(true);
        expect(contextDiag[0].sourceFound).toBe(false);
        expect(contextDiag[0].blockReason).toBeNull();
      });

      it("context-derived fallback reduces execution_blocking defects for InvokeWorkflowFile.WorkflowFileName in enforcer", () => {
        catalogServiceImport.load();
        if (!catalogServiceImport.isLoaded()) return;
        const xamlWithMissingFilename = makeValidXaml("Main",
          '<ui:InvokeWorkflowFile DisplayName="Invoke Process Data" />'
        );
        const entries = [{ name: "Main.xaml", content: xamlWithMissingFilename }];
        const result = enforceRequiredProperties(entries, true);
        const invokeDefects = result.unresolvedRequiredPropertyDefects.filter(
          d => d.activityType === "InvokeWorkflowFile" && d.propertyName === "WorkflowFileName"
        );
        expect(invokeDefects.length).toBe(0);

        const contextDerivedBindings = result.requiredPropertyBindings.filter(
          b => b.activityType === "InvokeWorkflowFile" && b.propertyName === "WorkflowFileName" && b.sourceBinding === "context-derived"
        );
        expect(contextDerivedBindings.length).toBe(1);
        expect(contextDerivedBindings[0].resolvedValue).toContain("AutoGenerated_Invoke_");
        expect(contextDerivedBindings[0].resolvedValue).toContain("Invoke_Process_Data");
      });
    });
  });
