...
- **Infrastructure Workflow Dedup**: `canonicalizeWorkflowName()` strips brackets, quotes, .xaml suffixes and normalizes to lowercase before matching against the canonical infrastructure set, preventing phantom duplicate workflows like `[Init.xaml].xaml`.
