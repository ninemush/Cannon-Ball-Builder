...
-   **Process Map Auto-Clone Guard**: To-be auto-clone from as-is uses a DB-first check: always queries to-be nodes before cloning. If to-be already has nodes, clone is suppressed — no dependency on in-memory state. Bulk endpoint acquires `toBeGenerationLocks` BEFORE clearing, preventing GET auto-clone race condition. Write-side mutual exclusion via `bulkWriteLocks` serializes concurrent bulk requests for the same idea+view. Approval-triggered clone also checks for existing to-be content before cloning.
-   **AS-IS Agent Node Enforcement**: Three-layer protection ensures agent node types (agent-task, agent-decision, agent-loop) never appear in as-is views. (1) AI prompt explicitly forbids agent types in AS-IS with self-check rules. (2) Client-side `splitAgentNodesFromAsIs()` in `step-parser.ts` moves any agent nodes from as-is groups into to-be groups. (3) Server-side bulk endpoint downgrades agent-task→task, agent-decision→decision, agent-loop→task when viewType is "as-is". Startup cleanup also downgrades any pre-existing agent nodes in as-is views.
-   **Deploy Parallelization**: `deployAllArtifacts` in `uipath-deploy.ts` groups independent UiPath API calls into parallel batches via `Promise.all`. Group 1: queues, assets, storage buckets, environments, machines, robots. Group 2: triggers, Action Center, DU, test cases, agent artifacts. Group 3: requirements, test sets (after test provision). Reduces deploy time by running independent provisions concurrently.
