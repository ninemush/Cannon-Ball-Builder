...
- **V2 UiPath Integration Layer**: Hardened auth module (`server/uipath-auth.ts`) with **multi-resource token architecture** — UiPath's identity server requires separate tokens per resource (OR.*, TM.*, Du.*, PM.*, DataFabric.* scopes cannot be combined in one token request). Five independent token types: OR token (`getToken()`/`getHeaders()`) for Orchestrator API, TM token (`getTmToken()`/`getTmHeaders()`) for Test Manager, DU token (`getDuToken()`) for Document Understanding, PM token (`getPmToken()`) for Platform Management/Robot Accounts, DF token (`getDfToken()`) for Data Service/DataFabric. All have singleton caching via `tokenCache: Partial<Record<ResourceType, CachedToken>>`, proactive 60s-before-expiry refresh, 401 auto-retry with token-type-aware invalidation, and secret masking. `makeAuthenticatedRequest()` auto-selects token via `detectResourceType()` based on URL patterns: `testmanager_/`→TM, `du_/`→DU, `dataservice_/`→DF, `identity_/`+PM→PM, default→OR. `tryAcquireResourceToken()` tests scope validity per resource. Service-specific base URL helpers: `getBaseUrl` (Orchestrator `orchestrator_`), `getTestManagerBaseUrl` (Test Manager `testmanager_`), `getDuBaseUrl` (DU `du_`), `getDataServiceBaseUrl` (Data Service `dataservice_`). `verifyUiPathScopes()` probes each service with its dedicated token and reports availability. `probeServiceAvailability()` returns `ServiceAvailabilityMap` with fields: `configured`, `orchestrator`, `actionCenter`, `testManager`, `documentUnderstanding`, `dataService`, `platformManagement`, `environments`, `triggers`. Action Center uses Orchestrator OData API (`{orchBase}/odata/TaskCatalogs`) with OR.Tasks scopes. DU provisioning uses DU token with `api-version=1` query parameter. PM robot account provisioning uses centralized `getPmToken()`. UI scope verification groups scopes by resource type (OR, TM, Du, PM, DataFabric) with color-coded badges. Typed Orchestrator API client (`server/orchestrator-client.ts`) with exponential backoff retry (3x), 30s timeout, and full method coverage. Prerequisite checker (`server/prerequisite-checker.ts`) with 6 checks returning blocking/warning/pass statuses. New API endpoints: `/api/uipath/health`, `/api/uipath/live-ops`, `/api/uipath/diagnostics`, `/api/uipath/prerequisites`, provisioning status/emergency-stop, action center tasks, test results.
...
UiPath's identity server (`cloud.uipath.com/identity_/connect/token`) cannot mix scopes from different resource types in a single token request (returns `invalid_scope`). The auth module maintains 5 independent cached tokens:
...
Invalid on this tenant: `PIMSDataFabric.Schema.Read` (returns `invalid_scope`).

Key endpoint patterns:
- DU: `/du_/api/framework/projects?api-version=1` (403 = available, needs folder header)
- DataFabric: `/dataservice_/swagger/index.html` (200 = provisioned)
- PM: Token acquisition success = available (Identity API robot endpoints return HTML)
- TM: `/testmanager_/api/v2/Projects` (200 with projects)