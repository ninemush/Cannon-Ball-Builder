...
-   **Process Map Edge Routing**: Custom edge rendering uses sourceHandle differentiation for decision/agent-decision nodes (bottom/right/left handles for different branches), increased per-sibling offsets (20-28px), and staggered SmoothStep routing offsets to prevent connector line overlap while keeping the layout compact.
