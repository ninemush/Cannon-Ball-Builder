...
const ORTHOGONAL_25: RenderConfig = {
  label: "ORTHOGONAL-25",
  taskWidth: 240,
  taskHeight: 70,
  startEndSize: 44,
...
const ORTHOGONAL_100: RenderConfig = {
  label: "ORTHOGONAL-100",
  taskWidth: 240,
  taskHeight: 70,
  startEndSize: 44,
  decisionSize: 52,
...
function generate100NodeData(): { nodes: MapNode[]; edges: MapEdge[] } {
  const allNodes: MapNode[] = [];
  const allEdges: MapEdge[] = [];
  const copies = 4;
  let prevEndNodeId: number | null = null;

  for (let copy = 0; copy < copies; copy++) {
    const nodeOffset = copy * 10000;
    const edgeOffset = copy * 10000;
    const orderOffset = copy * TEST_NODES.length;

    const endNodeIds: number[] = [];
    const startNodeId = TEST_NODES[0].id + nodeOffset;

    for (const node of TEST_NODES) {
      const newId = node.id + nodeOffset;
      allNodes.push({
        ...node,
        id: newId,
        name: `[${copy + 1}] ${node.name}`,
        orderIndex: (node.orderIndex || 0) + orderOffset,
        nodeType: copy === 0 && node.nodeType === "start" ? "start" : 
                  copy === copies - 1 && node.nodeType === "end" ? "end" :
                  node.nodeType === "start" ? "task" :
                  node.nodeType === "end" ? "task" : node.nodeType,
      });
      if (node.nodeType === "end") endNodeIds.push(newId);
    }
    ...
    for (const edge of TEST_EDGES) {
      allEdges.push({
        ...
        sourceNodeId: edge.sourceNodeId + nodeOffset,
        targetNodeId: edge.targetNodeId + nodeOffset,
      });
    }

    if (prevEndNodeId !== null) {
      allEdges.push({
        id: 90000 + copy,
        sourceNodeId: prevEndNodeId,
        targetNodeId: startNodeId,
        label: "",
      });
    }

    prevEndNodeId = endNodeIds.length > 0 ? endNodeIds[0] : null;
  }
...
function computeLayout(
  ...
  for (const le of layoutEdges) {
    ...
    if (srcNode && tgtNode) {
      le.points = computeOrthogonalEdgePoints(srcNode, tgtNode, le.sourceHandle, le.isDecisionSource || false, cfg, nodeById, layoutNodes);
    }
  }
...
function computeBaselineLayout(
  nodes: MapNode[],
  edges: MapEdge[],
  cfg: RenderConfig,
): { layoutNodes: LayoutNode[]; layoutEdges: LayoutEdge[] } {
  ...
  for (const le of layoutEdges) {
    ...
    const tgtNode = nodeById[le.target];
    if (srcNode && tgtNode) {
      le.points = computeBaselineEdgePoints(srcNode, tgtNode, le.sourceHandle, le.isDecisionSource || false, cfg, nodeById, layoutNodes);
    }
  }
...
function computeBaselineEdgePoints(
  srcNode: LayoutNode,
  tgtNode: LayoutNode,
  ...
  if (!isDecisionSource) {
    ...
    if (ty < sy) {
      const goRight = tx > cx;
      const allEdges = allNodes.map(n => ({ left: n.x, right: n.x + n.width }));
      const maxRight = Math.max(...allEdges.map(e => e.right));
      const minLeft = Math.min(...allEdges.map(e => e.left));
      const routeX = goRight ? maxRight + 40 : minLeft - 40;
      return [
        { x: sx, y: sy }, { x: sx, y: sy + 20 },
        { x: routeX, y: sy + 20 }, { x: routeX, y: ty - 20 },
        { x: tx, y: ty - 20 }, { x: tx, y: ty },
      ];
    }
    if (Math.abs(sx - tx) < 8) {
  ...
  if (sourceHandle === "left") {
    ...
    if (ty <= cy) {
      return [
        { x: sx, y: sy }, { x: routeX, y: sy },
        { x: routeX, y: ty - 20 }, { x: tx, y: ty - 20 },
        { x: tx, y: ty },
      ];
    }
    return [
      { x: sx, y: sy }, { x: routeX, y: sy },
      { x: routeX, y: midY }, { x: tx, y: midY },
      { x: tx, y: ty },
    ];
  }
...
function computeOrthogonalEdgePoints(
  srcNode: LayoutNode,
  tgtNode: LayoutNode,
  sourceHandle: string | undefined,
  isDecisionSource: boolean,
  ...
  if (!isDecisionSource) {
    ...
    if (ty < sy) {
      const goRight = tx > cx;
      const allBounds = allNodes.map(n => ({ left: n.x, right: n.x + n.width }));
      const maxRight = Math.max(...allBounds.map(e => e.right));
      const minLeft = Math.min(...allBounds.map(e => e.left));
      const routeX = goRight ? maxRight + 50 : minLeft - 50;
      const dropY = sy + 25;
      const riseY = ty - 25;
      return [
        { x: sx, y: sy },
  ...
  if (sourceHandle === "left") {
    const sx = cx - halfDiamond;
    const sy = cy;
    const routeX = Math.min(sx - 40, tgtNode.x - 25);
  ...
  if (sourceHandle === "right") {
    const sx = cx + halfDiamond;
    const sy = cy;
    const routeX = Math.max(sx + 40, tgtNode.x + tgtNode.width + 25);
...
async function renderWithConfig(
  ...
  const { layoutNodes, layoutEdges } = useOrthogonal
    ? computeLayout(nodes, edges, cfg)
    : computeBaselineLayout(nodes, edges, cfg);
  ...
  const adjustedNodes = layoutNodes.map((n) => ({ ...n, x: n.x + offsetX, y: n.y + offsetY }));
  ...
  const modeLabel = useOrthogonal ? "ORTHOGONAL" : "BASELINE (bezier)";
  const titleFontSize = 12;
  const titleSvg = `<text x="${svgWidth / 2}" y="${30}" text-anchor="middle" fill="#71717a" font-size="${titleFontSize}" font-weight="700" font-family="system-ui, sans-serif">${cfg.label} [${modeLabel}]: ${nodes.length} nodes, nodesep=${cfg.nodesep}, ranksep=${cfg.ranksep}, cornerR=${cfg.edgeCornerRadius}</text>`;
...
async function main() {
  ...
  console.log("=== Orthogonal Connector Test ===\n");

  console.log("1. Rendering BASELINE (current bezier, 25 nodes)...");
  const baseline = await renderWithConfig(TEST_NODES, TEST_EDGES, BASELINE, false);
  fs.writeFileSync(path.join(outDir, "baseline.svg"), baseline.svgContent);
  fs.writeFileSync(path.join(outDir, "baseline.png"), baseline.pngBuffer);
  console.log(`   SVG: ${baseline.width}x${baseline.height}, PNG: ${(baseline.pngBuffer.length / 1024).toFixed(0)}KB`);

  console.log("\n2. Rendering ORTHOGONAL (25 nodes, nodesep=160, cornerR=12)...");
  const ortho25 = await renderWithConfig(TEST_NODES, TEST_EDGES, ORTHOGONAL_25, true);
  fs.writeFileSync(path.join(outDir, "orthogonal-25-nodes.svg"), ortho25.svgContent);
  fs.writeFileSync(path.join(outDir, "orthogonal-25-nodes.png"), ortho25.pngBuffer);
  console.log(`   SVG: ${ortho25.width}x${ortho25.height}, PNG: ${(ortho25.pngBuffer.length / 1024).toFixed(0)}KB`);

  const { nodes: nodes100, edges: edges100 } = generate100NodeData();
  console.log(`\n3. Rendering ORTHOGONAL (${nodes100.length} nodes, ${edges100.length} edges)...`);
  const ortho100 = await renderWithConfig(nodes100, edges100, ORTHOGONAL_100, true);
  fs.writeFileSync(path.join(outDir, "orthogonal-100-nodes.svg"), ortho100.svgContent);
  fs.writeFileSync(path.join(outDir, "orthogonal-100-nodes.png"), ortho100.pngBuffer);
  console.log(`   SVG: ${ortho100.width}x${ortho100.height}, PNG: ${(ortho100.pngBuffer.length / 1024).toFixed(0)}KB`);

  console.log("\n=== Summary ===");
  console.log(`  baseline.png          : ${baseline.width}x${baseline.height}, ${(baseline.pngBuffer.length / 1024).toFixed(0)}KB (25 nodes, bezier connectors)`);
  console.log(`  orthogonal-25-nodes.png : ${ortho25.width}x${ortho25.height}, ${(ortho25.pngBuffer.length / 1024).toFixed(0)}KB (25 nodes, orthogonal connectors)`);
  console.log(`  orthogonal-100-nodes.png: ${ortho100.width}x${ortho100.height}, ${(ortho100.pngBuffer.length / 1024).toFixed(0)}KB (${nodes100.length} nodes, orthogonal connectors)`);

  const widthRatio25 = (ortho25.width / baseline.width).toFixed(2);
  const heightRatio25 = (ortho25.height / baseline.height).toFixed(2);
  const sizeRatio25 = (ortho25.pngBuffer.length / baseline.pngBuffer.length).toFixed(2);
  console.log(`\n  25-node comparison (ortho vs baseline):`);
  console.log(`    Width:  ${widthRatio25}x (${baseline.width} -> ${ortho25.width})`);
  console.log(`    Height: ${heightRatio25}x (${baseline.height} -> ${ortho25.height})`);
  console.log(`    PNG:    ${sizeRatio25}x (${(baseline.pngBuffer.length / 1024).toFixed(0)}KB -> ${(ortho25.pngBuffer.length / 1024).toFixed(0)}KB)`);

  const pddScaleW = 580;
  const scale100 = pddScaleW / ortho100.width;
  const pddH100 = Math.round(ortho100.height * scale100);
  console.log(`\n  100-node at PDD scale (${pddScaleW}px width):`);
  console.log(`    Scaled height: ${pddH100}px`);
  console.log(`    PNG file size: ${(ortho100.pngBuffer.length / 1024).toFixed(0)}KB`);
  console.log(`    Readable: ${pddH100 < 8000 ? "YES (within reasonable PDD page limits)" : "CAUTION (very tall, may need pagination)"}`);
